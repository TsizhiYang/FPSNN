...
class ActFun(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output * (1 / lens) * (ActFun.apply(input + lens / 2) - ActFun.apply(input - lens / 2))
...
class SNN(nn.Module):
    ...
    def __init__(self, time_window=40,
        ...
        def encode(x, time_step):
            ...
            return ActFun.apply(x - thresh)
        self.encoder = encode                                   # AD encoder

        self.linear = nn.Linear(encode_len, 16)         
        self.decoder1 = nn.Linear(16, 8)                      
        self.decoder2 = nn.Linear(8, out_dim)                  
    ...
    def mem_update(self, operation, pre_spike, memory, spiked, decay=decay):
        ...
        spike = ActFun.apply(memory - LIF_thresh)
